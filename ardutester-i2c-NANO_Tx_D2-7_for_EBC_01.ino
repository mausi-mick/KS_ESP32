...
//#define uart_newline()     Serial.println()  //nl
...
long xreadSupply() {
  ...
  result =   (n * 1007500L) / result; // Back-calculate AVcc in mV
...
long readSupply() {
  ...
  result = ADCL;
  result |= ADCH << 8;
  result = 1007500L / result; // Back-calculate AVcc in mV
...
void isr_Switch() {   //?? push-button pressed
  times_n = millis();
  if (times_n < times_o + 30)  return;   // 15  
  times_o = times_n;
  s_switch = 1;
...
{
  Serial.begin(9600);
 // Serial.println("Tran-Tester");
  pinMode(pin_ADC_SW, OUTPUT);
  digitalWrite(pin_ADC_SW,LOW);   // disconnect ADC0,1,2 from TP1,2,3
  pinMode(pin_PUSHBUT, INPUT_PULLUP);    // D0 / RX TestKeyPin
// pinMode(pin_PUSHBUT, INPUT_PULLUP);  // D2 // TestKeyPin
// attachInterrupt(digitalPinToInterrupt(pin_PUSHBUT), isr_Switch, FALLING); //??
...
void expPins(int8_t tt_ct_mode) {
 
  if (tt_ct_mode == 1) {  // TranTester
    // disconnect matrix
    pinMode(2,INPUT);   
    pinMode(3,INPUT);
    pinMode(4,INPUT);
    ...
    pinMode(13,INPUT); // ??      
    delay(10);
    digitalWrite(pin_ADC_SW,HIGH);  // connect TP1,2,3 with ADC0,1,2
    delay(40);
      
  }
  if (tt_ct_mode == 2) {  // CurveTracer
  // Serial.println(2);
    digitalWrite(pin_ADC_SW,LOW);
    delay(40);
...
/*
  ON_DDR = 1;
 // ON_PORT = 1;   //??0? 0 for no i2c-Displays
...
void batCheck()  { // Battery check is selected
...
{
    digitalWrite(pin_ADC_SW,LOW);
    TranTest();                    // detect device-type and pin assignment
    digitalWrite(pin_ADC_SW,LOW);  // disconnect ADC0,1,2 from TP1,TP2,TP3
    if (kind_nr > 0) {             // for CurveTracer: connect TP1,TP2,TP3 to E,B or C
      switchMatrix(kind_nr,intface_t[3],intface_t[4],intface_t[5]);  // 0....8, Pin1, Pin2, Pin3(E,B,C etc.)
  /*    uart_putx(sw1+48);    // Pin: 1...9 int(0...9) to ascii
      uart_putx('\n');
      uart_putx(sw2+48);    // Pin: 1...9 int(0...9) to ascii
      uart_putx('\n');
      uart_putx(sw3+48);    // Pin: 1...9 int(0...9) to ascii
      uart_putx('\n'); */
      disconnectPins_EBC();
      delay(2000);
    }  
...
start:
  ...
  s_vgs = 0;
  lcd_clear();
  lcd_string("TranTester start");
 
  lcd_set_cursor(1, 0);
  lcd_string("press pushbutton");
  delay(1000);
    
  int s_button = 1;
  while(s_button==1) {
    
    s_button = digitalRead(pin_PUSHBUT);
    delay(10);
  }
  lcd_set_cursor(1, 0);
  lcd_string("T.tester started");
  delay(2000);
  lcd_clear();
  ...
  PartReady    = 0;
  PartMode     = 0;
  for (int8_t ilx=0;ilx<6;ilx++) {
    intface_t[ilx] = ' ';
    pinMode(ilx+2,INPUT);   // open switches in matrix
  }
 ...
 #ifdef WITH_UART
  // uart_newline();   // start of new measurement  //nl
 #endif
 ...
 #ifdef BAT_CHECK   // Battery check is selected
  batCheck();
 #else
    lcd_fix2_string(VERSION_str); // if no Battery check, Version 1.08 ... in row 1
 #endif   // end BAT_CHECK
  ...
  if(PartFound == PART_DIODE) {
    if(NumOfDiodes == 1) {    // single Diode
      ...
      #endif
      if      (intface_t[3] == 'A') kind_nr = 4;
      else if (intface_t[3] == 'K') kind_nr = 8;
      UfOutput(0x70);
    ...
    if(PartMode&1) {
      ...
      intface_t[1] = 'P'; // P-channel
      kind_nr = 6;  // preset as P_Mos
     
    } else {
      lcd_data('N'); // N-channel
      intface_t[1] = 'N'; // N-channel
      kind_nr = 2;  // preset as N_Mos
    ...
    if (tmp == (PART_MODE_N_D_MOS/2)) {
      lcd_data('D'); // N-D
      intface_t[2] = 'D'; // Depletion
      kind_nr = 9; // handle depletion-Mosfet like JFET #########
     
    }
    if (tmp == (PART_MODE_N_E_MOS/2)) {
      lcd_data('E'); // N-E
      intface_t[2] = 'E'; // Enhancement
    }

    if (tmp == (PART_MODE_N_JFET/2)) {
      lcd_fix_string(jfet_str);         // "JFET"
      intface_t[0] = '2';    // class: JFET
      kind_nr = kind_nr + 1; // change from preset Mosfet to JFET
      
     
    } else {
      lcd_fix_string(mosfet_str);       // "-MOS "
      intface_t[0] = '1';  // class: MOSFET
    }
  ...
  //    Serial.println(trans.uBE[1]);  //#########
      if (trans.uBE[1] > 550) s_vgs = 1; // > 4V
...
end2:
 ...
 // lcd.clear();     // clear total line test  CT CT <ct test ########################################################
 // lcd_line1();      // Cursor to column 1, row 1
 // lcd_line2();  // 2 row 

 // for (int8_t ilx=0;ilx<6;ilx++) {
 //    lcd_data(intface_t[ilx]);    //ct test ####################################
 // }
  //   CT CT
  /*  test
  lcd_line2();      // Cursor to column 1, row 1
  lcd_string("Cl ");
  lcd.write(intface_t[0]);//CurDUTclass+48);  // class // 1=Diode,2=npn/pnp,3=Mosfet,4=j-Fet
  lcd_string(" kind ");
  lcd.write(curkind+48);      // ascII
  lcd.write(' ');
  lcd.write(intface_t[3]);8z
  lcd.write(intface_t[4]);
  lcd.write(intface_t[5]);
  */
  if (intface_t[3] == 'A' and intface_t[4] == 'K'){
    kind_nr = 8;  // P-Diode
   
  }
  if (intface_t[4] == 'A' and intface_t[3] == 'K'){
    kind_nr = 4;  // N-Diode
  
  }

  uart_putc(' ');
  uart_putc(intface_t[0]);
  uart_putc(intface_t[1]);
  uart_putc(intface_t[2]);
  uart_putc(intface_t[3]);
  uart_putc(intface_t[4]);
  uart_putc(intface_t[5]);
  uart_putc(intface_t[6]);
  if (s_auto ==1) uart_putx(kind_nr+65);    // int(0...9) to ascii  'A','B',...'J' ########################
  else uart_putx(kind_nr+48);    // int(0...9) to ascii '0'...'9' ##########################################
  uart_putx('\n');
/*  
// set 4-bit-dac  ############################################
   if      (kind_nr == 1) {  // npn
    mcp.setPinX(4,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 2) {  // n-Mosfet
    mcp.setPinX(5,B,OUTPUT,HIGH);
  }
 
  else if (kind_nr == 3) {  // n-jFet
    if (intface_t[2] == 'D') {  // depletion-Mosfet
      mcp.setPinX(4,B,OUTPUT,HIGH);
      mcp.setPinX(5,B,OUTPUT,HIGH);
     
    }
    else mcp.setPinX(6,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 4) {  // n-Diode
    mcp.setPinX(5,B,OUTPUT,HIGH);
    mcp.setPinX(6,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 5) {  // pnp
    mcp.setPinX(7,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 6) {  // p-Mosfet
    mcp.setPinX(5,B,OUTPUT,HIGH);
    mcp.setPinX(7,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 7) {  // n-jFet
    mcp.setPinX(6,B,OUTPUT,HIGH);
    mcp.setPinX(7,B,OUTPUT,HIGH);
  }
  else if (kind_nr == 8) {  // p-Diode
    mcp.setPinX(5,B,OUTPUT,HIGH);
    mcp.setPinX(6,B,OUTPUT,HIGH);
    mcp.setPinX(7,B,OUTPUT,HIGH);
  }
  */
  if (kind_nr == 1 or kind_nr == 5) intface_t[0] = '0';  // bipolar
  ///  ###test###################################################################
  ...
  /* intface_t[0] = '2';
   ...
   intface_t[4] = 'C';
   intface_t[5] = 'E';
   Serial.print(" nach end: "); Serial.println(intface_t[0]);*/
  // if (intface_t[0] >= '0' and intface_t[0] <= '2')  // class: 0=bipolar,1=Mosfet,2=jFet 
 //  Serial.print(" kind_nr "); Serial.print(kind_nr);Serial.print(" intface_t[0] "); Serial.println(intface_t[0]);
  
 /*
  s_switch = 0;
  int ict = 0;
  while (s_switch == 0) {
    uart_putx(kind_nr+48);    // int(0...9) to ascii
    uart_putx('\n');
    delay(1);
    ict++;
    if (ict > 100) break;
  ...
  */
  delay(2000);  //####################################################
  if (kind_nr > 0) return; // curve-tracer  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CT CT CT CT #############################################
  delay(200);
...
void switchMatrix(uint8_t kind_nr,char pin1,char pin2,char pin3)  {// {0=tkNothing,1=tkNPN,2=tkNMOSFET,3=tkNJFET,4=tkNDiode,5=tkPNP,6=tkPMOSFET,7=tkPJFET,8=tkPDiode};
    sw1 = 0;
    sw2 = 0;
    sw3 = 0;
    ...
    pinMode(11,INPUT); 
    pinMode(13,INPUT);    
    if (kind_nr == 4 or kind_nr == 8)   { // diode
      if (pin1 == 'K' ) {  // "n-Diode"
        pin1 = 'E';
        pin2 = 'B';  // ??
        pin3 = 'C';     
    ...
    if (kind_nr == 2 or kind_nr == 3 or kind_nr == 6 or kind_nr == 7 or kind_nr == 9) {  // change Fet-Pins G,D,S to bipolar-Pins B,C,E 
      if (s_vgs == 1) {
        char tp[4];
        if      (pin1 == 'S' and pin2 == 'D') {
          pin1 = 'D';
          pin2 = 'S'; 
        }
        else if (pin1 == 'S' and pin3 == 'D') {
          pin1 = 'D';
          pin3 = 'S'; 
        }
        else if (pin2 == 'S' and pin3 == 'D') {
          pin2 = 'D';
          pin3 = 'S'; 
        }
        else if (pin2 == 'S' and pin1 == 'D') {
          pin2 = 'D';
          pin1 = 'S'; 
        }
        else if (pin3 == 'S' and pin1 == 'D') {
          pin3 = 'D';
          pin1 = 'S'; 
        }
        else if (pin3 == 'S' and pin2 == 'D') {
          pin3 = 'D';
          pin2 = 'S'; 
        }
    //    Serial.print("s-vgs pin1 ");Serial.print(pin1);Serial.print(" pin2 ");Serial.print(pin2);Serial.print(" pin3 ");Serial.println(pin3);
        s_vgs = 0;
      }
      if      (pin1 == 'G') {  
        pin1 = 'B';
    ...
    if (pin1 == 'E' and pin2 == 'B' and pin3 == 'C') {  // EBC SGD 159 -> 2613
    ...
    if (pin1 == 'E' and pin2 == 'C' and pin3 == 'B') {  // ECB SDG 168 -> 2711
      sw1 = 1;
      sw2 = 6;
      sw3 = 8;
      return;
    }
    if (pin1 == 'B' and pin2 == 'C' and pin3 == 'E') {  // BCE GDS 267 -> 379
      sw1 = 2;
      sw2 = 6;
      sw3 = 7;
      return;
    } 
    if (pin1 == 'B' and pin2 == 'E' and pin3 == 'C') {  // BEC GSD 249 -> 3513
      sw1 = 2;
      sw2 = 4;
      sw3 = 9;
      return;
    }   
    if (pin1 == 'C' and pin2 == 'B' and pin3 == 'E') {  // CBE DGS 357 -> 469
      sw1 = 3;
      sw2 = 5;
      sw3 = 7;
      return;
    }   
    if (pin1 == 'C' and pin2 == 'E' and pin3 == 'B') {  // CEB DSG 348 -> 4511
      sw1 = 3;
      sw2 = 4;
      sw3 = 8;
      return;
...
void setOutput(int pin_H) {  
  pinMode(pin_H,OUTPUT);
  digitalWrite(pin_H,HIGH);
 // Serial.println(pin_H);
 
}  

void disconnectPins_EBC() {
   digitalWrite(pin_ADC_SW,LOW);// disconnect ADC0,1,2 from TP1,2,3
   pinMode(14,INPUT);  // disconnect ADC0,1,2 from TP1,2,3
   pinMode(15,INPUT);  
   pinMode(16,INPUT);  
   delay(5);
 
  
   if      (sw1 >= 1 and sw1 <= 6) setOutput(sw1+1);    // D2...D7
   else if (sw1 == 7) setOutput(9);     // D9   // A?
   else if (sw1 == 8) setOutput(11);    // D11  // B?
   else if (sw1 == 9) setOutput(13);    // D13  // C?

  
   
   if      (sw2 >= 1 and sw2 <= 6) setOutput(sw2+1);    // D2...D7
   else if (sw2 == 7) setOutput(9);     // D9  // A?
   else if (sw2 == 8) setOutput(11);    // D11 // B?
   else if (sw2 == 9) setOutput(13);    // D13 // C?

  
   if      (sw3 >= 1 and sw3 <= 6) setOutput(sw3+1);    // D2...D7
   else if (sw3 == 7) setOutput(9);     // D8  // A
   else if (sw3 == 8) setOutput(11);    // D11 // B
   else if (sw3 == 9) setOutput(13);    // D13 // C
...
void PinLayout(char pin1, char pin2, char pin3) {
  // pin1-3 is EBC or SGD or CGA
    ...
    for (ii=0;ii<3;ii++) {  // Output Character in right order
      if (ii == trans.e) {
        lcd_data(pin1); 
        intface_t[3+ii] = pin1;
      }
      ...
      ii = 3;
      while (ii != 0) { // Output Character in right order
        ii--;
        if (ii == trans.e) {
          lcd_data(pin1); 
          intface_t[3+ii] = pin1;
  ...
  */
  ...
  if(adc.lp_otr > 455) {   // there is more than 650uA current without TristatePin current 
    ...
    if (adc.hp1>(adc.hp2+488)) {
      ...
      //  PartMode = PART_MODE_P_D_MOS; // Depletion-MOSFET  // not really in production  ####################
          PartMode = PART_MODE_N_JFET;//P_E_MOS; //          #########################################################
        } else {      // JFET (pn-passage between Gate and Source is conducting)
